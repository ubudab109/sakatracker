...
class BatchPaymentController extends Controller
{
    ...
    public function showBatchPayment($id)
    {
        ...
        if ($data['batch_payment']->batch_payment_invoice) {
            ...
            $checkBatchPaymentInvoices = BatchPaymentInvoice::where('batch_payment_id', $batch)
                ->where('document_status', '!=', 'ditolak')
                ->get();
        ...
        $data['batch_payment_invoices'] = [];
        $batch_payment_invoices = BatchPaymentInvoice::where('batch_payment_id', $id)
            ->get()
            ->map(function ($batch_payment) {
                // if ($batch_payment['document_status'] != 'ditolak') {
                // }
    ...
    public function updateStatusInvoiceBathPayment(Request $request, $id)
    {
        $request->validate([
            'status' => ['required'],
        ...
        $input = $request->all();
        $batchPaymentInvoice = BatchPaymentInvoice::find($id);
        if ($input['notes'] != '') {
            $batchPaymentInvoice->exchange_invoice()->update(['note' => $input['notes']]);
        }
